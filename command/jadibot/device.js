...
async function messageHandler(
  ...
  console.log(
    `Command: ${clientData.command} | From: ${message.sender} | Args: ${clientData.args.join(' ')}`,
  ...
  if (!allArgs[1]) {
    const getListDevice = await _mongo_JadibotDeviceSchema.find({
      ownerJadibotPhone: message.sender,
    });
  ...
  if (allArgs[1] === 'add') {
    ...
    const existingDevice = await _mongo_JadibotDeviceSchema.findOne({
      ownerJadibotPhone: message.sender,
    ...
    if (!allArgs.join().includes('grantedAccess2'))
      return rem.sendButtons(
        from,
        `*「 DEVICE 」*\n\nPilih jenis authentikasi yang ingin digunakan untuk perangkat ini.`,
        [
          {
            id: `${prefix}device add grantedAccess1 grantedAccess2 qr ${deviceName}`,
            text: 'Kode QR',
          },
          {
            id: `${prefix}device add grantedAccess1 grantedAccess2 code ${deviceName}`,
            text: 'Code',
          },
        ],
        { quoted: message }
    ...
    if (allArgs[4] === 'code' && message.collectMessage === undefined) {
      await addCollectorMessage(
        sender,
        from,
        `${prefix}device add grantedAccess1 grantedAccess2 code ${deviceName}`,
        Date.now() + toMs('1m'),
        true,
    ...
    const numberHp =
      allArgs[4] === 'code'
        ? (message.collectMessage.startsWith('08')
            ? message.collectMessage.replace('08', '628')
            : message.collectMessage
          )
    ...
    const positionDevice =
      (await _mongo_JadibotDeviceSchema
        .find({ ownerJadibotPhone: message.sender })
        .countDocuments()) + 1;
    return rem.reply(
      from,
  ...
  if (allArgs[1] === 'delete') {
    ...
    const deviceList = await _mongo_JadibotDeviceSchema.find({
      ownerJadibotPhone: message.sender,
    });
    ...
    await _mongo_JadibotDeviceSchema.deleteOne({
      apiKey: deviceToDelete.apiKey,
    });
  ...
  if (allArgs[1] && !isNaN(allArgs[1])) {
    ...
    if (deviceToStart?.settings?.pairMethod === 'qr')
      await rem.sendText(
        from,
        'Scan Kode QR Dibawah, cara lihat dibawah\n\nKembali ke beranda WhastApp\n*1. Titik Tiga*\n[ _Pojok kanan atas_ ]\n*2. Perangkat Tertaut*\n*3. Tautkan Perangkat*\n\n*Note :* _Jika hanya mempunyai hp 1, Buka link dibawah kode QR dengan perangkat lainnya_'
      );
    if (deviceToStart?.settings?.pairMethod === 'code')
      await rem.sendFile(
        from,
        './media/img/wamdcode.png',
        'wamdcode.png',
        'Masukkan Kode Dibawah, cara lihat digambar\n\nKembali ke beranda WhastApp\n*1. Titik Tiga*\n[ _Pojok kanan atas_ ]\n*2. Perangkat Tertaut*\n*3. Tautkan Perangkat*\n*4. Tautkan dengan nomor telepon saja [ _Bawah sendiri_ ]*\n5. *Ketikkan kode yang kirim oleh bot*',
        '',
        'imageMessage'
      );

    await startBot(
      deviceToStart.apiKey,
      deviceToStart.nameDevice,
      rem?.apiKey || rem,
      message.sender
    );
    return rem.reply(

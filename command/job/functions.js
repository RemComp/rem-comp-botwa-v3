...
const addJobXp = async (_userDb, userId, amount) => {
  ...
  if (
    _userDb.item?.jobBoost &&
    _userDb.item?.jobBoost?.time != 0 &&
    _userDb.item?.jobBoost?.xp != 0
  )
    pushedAmount = amount * _userDb.item.jobBoost.xp;
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $inc: { 'economy.job.xpjob': pushedAmount } }
  );
};
...
const addWork = async (userId, time) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'economy.worklimit': Date.now() + toMs(time) } }
  );
};

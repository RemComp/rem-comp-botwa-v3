...
const infinityNumber = 1.797693134862315e307;
const minInfinityNumber = -1.797693134862316e307;

//FUNCTION MONEY
const setMoney = async (userId) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'economy.money': 2000 } }
  );
};
...
const MinMoney = async (userId, amount, _userDb = undefined) => {
  const moneyDb = !_userDb
    ? await _mongo_UserSchema.findOne({ iId: userId })
    : _userDb;
  ...
  return !isFinite(moneyDb.economy.money - amount) ||
    isNaN(moneyDb.economy.money - amount)
    ? await _mongo_UserSchema.updateOne(
        { iId: userId },
        { $set: { 'economy.money': check } }
      )
...
const addMoney = async (userId, amount, _userDb = undefined) => {
  ...
  !isFinite(moneyDb.economy.money - amount) ||
  isNaN(moneyDb.economy.money - amount)
    ? await _mongo_UserSchema.updateOne(
        { iId: userId },
        { $set: { 'economy.money': check } }
...
const addMoneyLimitGive = async (_userDb, userId, to, amount) => {
  ...
  if (_limitGiveMoney != undefined) {
    ...
    if (_limitGiveIsAda != 'false') {
      ...
      await _mongo_UserSchema.updateOne(
        { iId: userId, 'limit.lpaym.id': to },
        { $inc: { 'limit.lpaym.$.limit': amount } }
      );
    } else {
      await _mongo_UserSchema.updateOne(
        { iId: userId },
        { $push: { 'limit.lpaym': { id: to, limit: amount } } }
      );
    }
  }
};

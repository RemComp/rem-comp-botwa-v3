...
const setTicTac_toe = async (userId, userId2) => {
  ...
  await _mongo_UserSchema.updateMany(
    { $or: [{ iId: userId }, { iId: userId2 }] },
    { $set: { tc: obj } }
  );
};
...
const getTicTac_toe = (_userDb) => {
  ...
  if (
    JSON.stringify(_tiktak?.tc) !== '{}' &&
    _tiktak?.tc != null &&
    _tiktak?.tc != undefined
  ) {
    // console.log(_tiktak)
...
const replaceTicTac_xo = async (userId, xo1) => {
  await _mongo_UserSchema.updateMany(
    { $or: [{ 'tc.player1': userId }, { 'tc.player2': userId }] },
    { $set: { 'tc.xo': xo1 } }
  );
};
...
async function handleGameWin(rem, from, gameState, winner, boardIcons, prefix) {
  ...
  await rem.sendTextWithMentions(
    from,
    `*Winner (${winIcon}) @${winnerPlayer.replace('@s.whatsapp.net', '')}*\nPoin : *+${poinRandomTictac}*\n\n❌ : @${gameState.xo[0].x.replace('@s.whatsapp.net', '')}\n⭕ : @${gameState.xo[0].o.replace('@s.whatsapp.net', '')}\n\n ${boardIcons.join('\n ')}`
  );
  await rem.sendButtons(
    from,
    'Link Button',
    [{ id: 'winner_profile', text: `${prefix}tc profile` }],
    '',
    ''
  );
  await _mongo_UserSchema.updateMany(
    { $or: [{ iId: gameState.player1 }, { iId: gameState.player2 }] },
    { $set: { tc: {} } }
  );
}
...
async function handleGameDraw(rem, from, gameState, boardIcons, prefix) {
  ...
  await rem.sendButtons(
    from,
    'Link Button',
    [{ id: 'draw_profile', text: `${prefix}tc profile` }],
    '',
    ''
  );
  await _mongo_UserSchema.updateMany(

...
const enableUserListItem = async (userId, item) => {
  const findItemEnable = await _mongo_UserSchema.findOne(
    { iId: userId },
    { 'item.item': 1 }
  );
  ...
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    {
      $pull: { 'item.item': { [item]: _item[item] } },
...
const disableUserListItem = async (userId, item) => {
  const findItemDisable = await _mongo_UserSchema.findOne(
    { iId: userId },
    { item: 1 }
  );
  ...
  await _mongo_UserSchema.updateOne(
    ...
    {
      $set: { [`item.${item}`]: {} },
      $push: { 'item.item': { [item]: _item[item] } },
    }
  );
  return true;
};
...
async function addJumItemJobBoost(_userDb, userId, amount) {
  ...
  if (allKeysDeactive.includes('jobBoost')) {
    await _mongo_UserSchema.updateOne(
      { iId: userId, 'item.item.jobBoost': { $exists: true } },
      { $inc: { 'item.item.$.jobBoost.xp': amount } }
    );
  } else {
    await _mongo_UserSchema.updateOne(
      { iId: userId },
      { $inc: { 'item.jobBoost.xp': amount } }
    );
  }
}
...
async function addJumItemLevel(_userDb, userId, amount) {
  ...
  if (allKeysDeactive.includes('level')) {
    await _mongo_UserSchema.updateOne(
      { iId: userId, 'item.item.level': { $exists: true } },
      { $inc: { 'item.item.$.level.level': amount } }
    );
  } else {
    await _mongo_UserSchema.updateOne(
      { iId: userId },
      { $inc: { 'item.level.level': amount } }
    );
  }
}

...
async function quizgameCheck(
  ...
  if (quotedMsg != null && quotedMsg.fromMe) {
    ...
    if (messageMathGame.includes(replyied_body)) {
      ...
      positionMathGame_player = _mathgame.findIndex(
        (el) => el?.mid == replyied_body
      );
    }
    ...
    if (responsesquizgame.includes(replyied_body)) {
      isMessageQuiz = true;
      positionQuizGame_player = _quizgame.findIndex(
        (el) => el?.mid == replyied_body
      );
    }
  }
  if (isMessageMathGame) {
    ...
    if (args0Filter_mathgame == jawabanMathGame_Answer.jawaban) {
      ...
      if (getUserItemId(_userDb) == undefined) {
        ...
        var amountXpssNambahMath = Math.floor(
          XPNambahMathGame * getItemLevel(_userDb).level
        );
      }
      ...
      await _mongo_GroupSchema.updateOne(
        { iId: from, 'game.mathgame.mid': quotedMsg.id },
        { $unset: { 'game.mathgame.$': 1 } }
      );
      await _mongo_GroupSchema.updateOne(
        { iId: from },
        { $pull: { 'game.mathgame': null } }
      );
    } else {
      rem.reply(from, 'SLAH');
    }
  } else if (isMessageQuiz) {
    if (_quizgame[positionQuizGame_player].game == 'fm100') {
      // fm100
      const jawabanQuizGame_answer = _quizgame[
        positionQuizGame_player
      ].jawaban.map((all) => all.toLowerCase().replace(/\s/g, ''));
      const jawabanArgs_chat = body.toLowerCase().replace(/\s/g, '');

      if (
        _quizgame[positionQuizGame_player]?.isJawab?.includes(jawabanArgs_chat)
      )
        return rem.reply(
          from,
          `sudah ada yang menjawab dengan jawaban yang sama sebelumnya`
        );
      if (jawabanQuizGame_answer.includes(jawabanArgs_chat)) {
        ...
        if (_quizgame[positionQuizGame_player].reason != undefined)
          rem.sendText(from, _quizgame[positionQuizGame_player].reason);

        await addLevelingXp(sender, XPNambahQuizGame);
        await _mongo_GroupSchema.updateOne(
          { iId: from, 'game.quizgame.mid': quotedMsg.id },
          {
            $set: { 'game.quizgame.$.isDone': true },
            $push: { 'game.quizgame.$.isJawab': jawabanArgs_chat },
          }
        );
      } else {
        rem.reply(from, 'SLAH');
      ...
      !_quizgame[positionQuizGame_player].isDone &&
      _quizgame[positionQuizGame_player].game == 'tebakgambar'
    ) {
      // tebakgambar
      const jawabanQuizGame_answer = _quizgame[positionQuizGame_player].jawaban
        .replace(/\s/g, '')
        .toLowerCase();
      const jawabanArgs_chat = body.toLowerCase().replace(/\s/g, '');
      ...
      !_quizgame[positionQuizGame_player].isDone &&
      _quizgame[positionQuizGame_player].game == 'tebakbendera'
    ) {
      // tebaklbendera
      const jawabanQuizGame_answer = _quizgame[positionQuizGame_player].jawaban
        .replace(/\s/g, '')
      ...
      !quizgame[positionQuizGame_player].isDone &&
      _quizgame[positionQuizGame_player].game == 'tbjapan'
    ) {
      //tebak huruf japan
      const jawabanQuizGame_answer = _quizgame[positionQuizGame_player].jawaban
        .replace(/\s/g, '')
      ...
      !_quizgame[positionQuizGame_player].isDone &&
      _quizgame[positionQuizGame_player].game == 'tebakwilayah'
    ) {
      // tebaklbendera
      const jawabanQuizGame_answer = _quizgame[positionQuizGame_player].jawaban
        .replace(/\s/g, '')
...
async function tictacGameCheck(
  rem,
  message,
  _userDb,
  _groupDb,
  ...
  if (
    checkTictacToePlayer.status === 'waiting' &&
    checkTictacToePlayer.player2 === sender
  ) {
    if (
      !isMedia &&
      type !== 'sticker' &&
      (args[0]?.toLowerCase() === '*y*' || args[0]?.toLowerCase() === 'y')
    ) {
      const playerListTictacToe = [
      ...
      await replaceTicTac_xo(sender, [
        { x: playerListTictacToe[0], o: playerListTictacToe[1] },
      ]);
      await replaceTicTac_turn(sender, playerListTictacToe[0]);

      rem.sendTextWithMentions(
        from,
  ...
  if (
    checkTictacToePlayer.status === 'playing' &&
    !isCmd &&
    !isMedia &&
    type !== 'sticker'
  ) {
    ...
    if (checkTictacToePlayer.turn !== sender) {
      ...
      return rem.sendTextWithMentions(
        from,
        `Sekarang giliran @${currentGame.turn.replace('@s.whatsapp.net', '')}`
      );
    }

    const selectedNumber = Math.floor(args[0]) - 1;
    if (isNaN(selectedNumber) || selectedNumber < 0 || selectedNumber > 8)
      return;
    ...
    // Switch turns
    const nextTurn =
      checkTictacToePlayer.turn === checkTictacToePlayer.player1
        ? checkTictacToePlayer.player2
        : checkTictacToePlayer.player1;

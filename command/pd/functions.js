...
*/
const setUserRoleplay_pasangan = async (
  userId,
  nama,
  gender,
  imgUrl,
...
const replaceUserRoleplayFood_pasangan = async (userId, CalculateAmount) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'rl.pd.food': CalculateAmount } }
  );
};

const multiplierHubunganBurukMin = 0.9;
const replaceUserRoleplayHubungan_pasangan = async (
  userId,
  CalculateAmount
) => {
  ...
  if (JSON.stringify(pd) != '{}') {
    ...
    await _mongo_UserSchema.updateOne(
      { iId: userId },
      { $set: { 'rl.pd.hubungan': inputDataValue } }
    );
  }
};
...
const replaceUserRoleplayMood_pasangan = async (userId, change) => {
  ...
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'rl.pd.mood': change } }
  );
};

const replaceUserRoleplayHubungan_pasangan_force = async (
  userId,
  CalculateAmount
) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'rl.pd.hubungan': CalculateAmount } }
  );
};
...
const replaceUserRoleplayUang_pasangan = async (userId, CalculateAmount) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'rl.pd.uang': CalculateAmount } }
  );
};
...
const replaceUserRoleplayStatus2_pasangan = async (userId, text) => {
  await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $set: { 'rl.pd.status2': text } }
  );
};

const replaceUserRoleplayValue_pasangan = async (
  userId,
  input,
  textOrNumber
) => {
...
const addUserRoleplayAnak_pasangan = async (
  userId,
  idAnak,
  nama,
  waktuLahir,

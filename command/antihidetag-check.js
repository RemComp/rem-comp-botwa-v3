...
function messageHandler (rem, message, _userDb, _groupDb, _mentionUserDb, clientData) {
    ...
    if((bodyMsgAntiHidetag != undefined && !bodyMsgAntiHidetag?.includes('@')) && (mentionedJidListAntiHidetag?.[0] != undefined)) {
        rem.reply(message.from, `*「 ANTI HIDETAG 」*\n\nKamu mengirimkan hidetag, maaf kamu di kick dari grup :(`)
        .then(async () => {
            await rem.deleteMessage(message.from, { id: message.id, remoteJid: message.from, fromMe: message.key.fromMe, participant : message.key.participant } )

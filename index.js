...
if (!process.env.PORT_START)
  return console.error(
    'PORT_START is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.PORT_WEB)
  return console.error(
    'PORT_WEB is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.PORT_GOLANG)
  return console.error(
    'PORT_GOLANG is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.PROCESS_COUNT)
  return console.error(
    'PROCESS_COUNT is not set in environment variables. Please set it before running the script.'
  );

if (!process.env.WEB_ADMIN_USERNAME)
  return console.error(
    'WEB_ADMIN_USERNAME is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.WEB_ADMIN_PASSWORD)
  return console.error(
    'WEB_ADMIN_PASSWORD is not set in environment variables. Please set it before running the script.'
  );

if (!process.env.SERVER_ID)
  return console.error(
    'SERVER_ID is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.MONGO_URI)
  return console.error(
    'MONGO_URI is not set in environment variables. Please set it before running the script.'
  );
if (!process.env.KEY_GOLANG)
  return console.error(
    'KEY_GOLANG is not set in environment variables. Please set it before running the script.'
  );
...
async function setupMultiCLient() {
  ...
  if (!process.env.pm_id) {
    console.error(
      'This script should be run using PM2. Please start it with "pm2 start index.js --name rem"'
    );
    process.exit(1);
  }
  ...
  // web interface
  const clientAllPort = parsedProcessRun.map((client) => {
    return {
      id: client.env.CLIENT_ID,
      name: client.env.CLIENT_NAME,
...
function checkOSGolangSupported() {
  const currentOS = golangSupportedOS.find(
    (os) => os.platform === process.platform
  );
  return currentOS;
}
async function initGolangEngine(clientAllPort = []) {
  ...
  if (!currentOS) {
    console.error(
      '+ Error when initialize whatsmeow engine, Err: ',
      'whatsmeowerr_unsupported_platform'
    );
    // process.exit(0);
  ...
  const parsedWEBHOOK_MAIN = JSON.stringify(
    clientAllPort.map(
      (client) => `http://localhost:${client.port}/webhook/golangengine`
    )
  );
  ...
  return new Promise((resolve, reject) => {
    exec(
      `pm2 start "${path.resolve(process.cwd() + '/' + fileNameGolangEngine)}" --name rem-comp-golang --update-env`,
      {
        cwd: process.cwd(),
        env: Object.assign(process.env, {
          ENGINE_GOLANG_KEY: process.env.KEY_GOLANG,
          ENGINE_GOLANG_PORT: process.env.PORT_GOLANG,
          WEBHOOK_MAIN: parsedWEBHOOK_MAIN,
          WHATSAPP_CONN_IDMSG: 'RMCP',
        }),
      },
      async (error) => {
        if (error) {
        ...
        await sleep(5000);
        axios
          .get(`http://localhost:${portGolang}/version`, { timeout: 30000 })
          .then((response) => {
  ...
  return new Promise((resolve, reject) => {
    const startPromises = processArr.map((client) => {
      return new Promise((res, rej) => {
        exec(
          `pm2 start ${client.script} --name ${client.name} --update-env --no-autorestart`,
          {
            cwd: process.cwd(),
          ...
          (error) => {
            if (error) {
              console.error(
                `+ Error when starting client ${client.name}, Err: `,
                'whatsmeowerr_err_start_client'
              );
              console.error(`exec error: ${error}`);
              // process.exit(0);
              execSync(`pm2 stop ${process.env.pm_id}`, { cwd: process.cwd() });
...
async function turnOffGolangEngine() {
  return new Promise(async (resolve, reject) => {
    const isExistGolangEngine = (await execSync(`pm2 list`))
      .toString()
      .includes('rem-comp-golang');
    if (isExistGolangEngine) {
      exec(
        `pm2 delete rem-comp-golang`,
        {
          cwd: process.cwd(),
        },
        async (error) => {
    ...
    if (filterClient.length > 0) {
      ...
      exec(
        ...
        (error) => {
          if (error) {
            console.error(
              '+ Error when turning off all client, Err: ',
              'whatsmeowerr_err_stop_all'
            );
            console.error(`exec error: ${error}`);
            // process.exit(0);
            execSync(`pm2 stop ${process.env.pm_id}`, { cwd: process.cwd() });
...
async function turnOffWebInterface() {
  return new Promise(async (resolve, reject) => {
    const isExistWebInterface = execSync(`pm2 list`)
      .toString()
    ...
    if (isExistWebInterface) {
      exec(
        ...
        (error) => {
          if (error) {
            console.error(
              '+ Error when turning off web interface, Err: ',
              'whatsmeowerr_err_stop_web'
            );
            console.error(`exec error: ${error}`);
            // process.exit(0);
            execSync(`pm2 stop ${process.env.pm_id}`, { cwd: process.cwd() });
...
async function getListPm2() {
  ...
  return new Promise((resolve, reject) => {
    exec(`pm2 jlist`, { cwd: process.cwd() }, (error, stdout, stderr) => {
      if (error) {
        console.error(
          '+ Error when getting list pm2, Err: ',
          'whatsmeowerr_err_list_pm2'
        );
        console.error(`exec error: ${error}`);
        return reject(error);
      }
      try {
        ...
        console.error(
          '+ Error when parsing pm2 list, Err: ',
          'whatsmeowerr_err_parse_pm2'
        );
        reject(e);
      }
    });

...
const userDataTypes = {
  ...
  uid: { type: Number, unique: true, immutable: true },
  isAdmin: Boolean,
  isPremium: Boolean,
  afk: Object,
  isBanned: Boolean,
  bannedReason: String,
  isSuperBanned: mongoose.Schema.Types.Mixed,
  listCmdBan: Array,
  timeBanned: Number,
  senderBanned: String,
  ...
  rl: Object,
  nametag: Object,
  'economy.level': Number,
  'economy.xp': Number,
  'economy.money': Number,
  'economy.moneyharam': Number,
  'economy.job': Object,
  'economy.worklimit': Number,
  economy: Object,
  lastAction: Object,
  ig: String,
  item: Object,
  ...
  discordId: String,
  email: String,
  collectMessage: Array,
  ai: Object,
  gameWeb: Object,
  rpg: Object,
  antispam: Object,
};
const _mongo_UserSchema = mongoose.Schema(userDataTypes, {
  collection: 'user',
});
...
// }
//create mongoose schema with the same structure as the object above
const _mongo_GroupSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      unique: true,
    ...
    prefix: String,
    muted: Object,
    isSubsAnimeNews: Boolean,
    isImageSafeSearch: Boolean,
    isSimsimi: Boolean,
    isNsfw: Boolean,
    isAntiBadword: Boolean,
    isAntiLink: Boolean,
    isAntiHidetag: Boolean,
    isJoinMsg: Boolean,
    isLeaveMsg: Boolean,
    isNoCmd: Boolean,
    rulesSet: String,
...
const _mongo_InvestHistorySchema = mongoose.Schema(
  {
    coin: {
      type: String,
      required: true,
...
// }
//create mongoose schema with the same structure as the object above
const _mongo_BotSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      unique: true,
...
//     content: [],
// }
const _mongo_CommandSchema = mongoose.Schema(
  {
    cId: {
      type: String,
      required: true,
...
//     name: 'XXX',
// }
const _mongo_ContactSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      required: true,
...
//     data: {},
// }
const _mongo_JadibotSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      unique: true,
 ...
 * }
 */
const _mongo_JadibotDeviceSchema = mongoose.Schema(
  {
    apiKey: {
      type: String,
      unique: true,
    ...
    settings: Object,
    owner: String,
    sourceJadibotApiKey: String,
    ownerJadibotPhone: String,
    isBotUtama: Boolean,
  },
  { collection: 'jadibot_device' }
);
...
//     from: 'XXX',
// }
const _mongo_RedeemSchema = mongoose.Schema(
  {
    code: {
      type: String,
      required: true,
...
//     contact: 300000000,
// }
const _mongo_CounterSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      required: true,
...
const _mongo_CommandMessageSchema = mongoose.Schema(
  {
    iId: {
      type: String,
      required: true,
...
const jadibotSchema = mongoose.model(
  'jadibot',
  _mongo_JadibotSchema,
  'jadibot'
);
const jadibotDeviceSchema = mongoose.model(
  'jadibot_device',
  _mongo_JadibotDeviceSchema,
  'jadibot_device'
);
const redeemSchema = mongoose.model(
  'redeem_db',
  _mongo_RedeemSchema,
  'redeem_db'
);
const counterSchema = mongoose.model(
  'counter',
  _mongo_CounterSchema,
  'counter'
);
const commandMessageSchema = mongoose.model(
  'command_message',
  _mongo_CommandMessageSchema,
  'command_message'
);
const investHistorySchema = mongoose.model(
  'invest_history',
  _mongo_InvestHistorySchema,
  'invest_history'
);

async function getNextSequenceValue(sequenceName) {
  const sequenceDocument = await _mongo_CounterSchema.findOneAndUpdate(
    { iId: sequenceName },
    { $inc: { count: 1 } },
    { new: true }
  );

...
async function requestToGolangEngine(path, data, options = {}) {
  try {
    if (!global.golangEngine.engineUrl) {
      global.log?.error(
        `Golang engine URL is not set. Please ensure the engine is initialized.`
      );
      return new Error('whatsmeowerr_engine_not_ready');
    }

    const response = await axios.post(
      `${global.golangEngine.engineUrl}${path}`,
      { accessKey: global.golangEngine.golangKey, data: JSON.stringify(data) },
      Object.assign({ validateStatus: () => true }, options)
    );
...
function formatRequiredDataClient(rem, _userDb, _groupDb, message) {
  ...
  const groupAdmins =
    message.isGroupMsg && _groupDb?.metadata
      ? _groupDb?.metadata.participants
          .filter((admn) => admn.admin != null)
          .map((admn2) => admn2.id)
      : '';
  const isGroupAdmins = message.isGroupMsg
    ? groupAdmins.includes(message.sender)
    : false;
  const isBotGroupAdmins = message.isGroupMsg
  ...
  if (message.isGroupMsg) {
    ...
    if (body != undefined ? body.startsWith('!') : false) {
      ...
      message.selectedButtonId.startsWith('!')
    ) {
      prefix = '!';
    } else if (
      message.selectedButtonId != undefined &&
      ...
      message.selectedRowId != undefined &&
      message.selectedRowId.startsWith('!')
    ) {
      prefix = '!';
    } else if (
      message.selectedRowId != undefined &&
  ...
  if (
    message.selectedButtonId != undefined &&
    message.selectedButtonId.startsWith(prefix)
  ) {
    allArgs = message.selectedButtonId.split(' ') || '';
  } else if (
    message.selectedRowId != undefined &&
    message.selectedRowId.startsWith(prefix)
  ) {
...
async function addCollectorMessage(
  userId,
  from,
  id,
  timeout,
  ...
  return await _mongo_UserSchema.updateOne(
    { iId: userId },
    { $push: { collectMessage: { id, from, timeout, isReply, messageReply } } }
  );
}

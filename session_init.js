...
async function initSession() {
  ...
  for (const device of getDeviceSameServer) {
    console.log(
      `[INIT] Starting bot for device: ${device.nameDevice} (${device.apiKey})`
    );
    const isErr = await startBot(
      device.apiKey,
      device.nameDevice,
      device.sourceJadibotApiKey,
      device.ownerJadibotPhone
    );
    if (isErr.error)
      console.error(
        `[INIT] Error starting bot for device ${device.nameDevice}:`,
        isErr.error
      );
  }
}
